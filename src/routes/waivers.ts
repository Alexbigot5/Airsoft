...
import type { AppBindings, AttendeeRow } from '../types';
import {
  ApiError,
  optionalString,
  requireDateOnly,
...
waivers.post('/waiver/attendee/:token', async (c) => {
  ...
  // Age is decided here, from the date of birth, against the day they will
  // actually play -- not from anything the client claims.
  const age = ageOn(dateOfBirth, attendee.event_starts_at);
  if (age < 0 || age > 120) {
  ...
  if (!waiver) throw new ApiError(500, 'waiver_insert_failed', 'Could not record the signature.');
  ...
  await c.env.DB.prepare(
    ...
    .bind(waiver.id, signerName, signerEmail, dateOfBirth, isMinor ? 1 : 0, attendee.id)
    .run();
